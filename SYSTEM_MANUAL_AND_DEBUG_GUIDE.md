...
| **Lệch dữ liệu khi dùng trên 2 máy khác nhau** | [`cloudSyncService.ts`](file:///d:/EngPractice/src/services/cloudSyncService.ts) / [`AppContext.tsx`](file:///d:/EngPractice/src/context/AppContext.tsx) | LocalStorage/Cookie độc lập từng máy + ghi đè đè dữ liệu cũ lên Cloud | Đã triển khai Smart Non-Destructive Merge, Hydration Gate (chống đè state rỗng) và Kênh lắng nghe Realtime Firebase Subscriptions cho Đề thi, Câu hỏi, Tài khoản và Tiến độ học sinh. |
